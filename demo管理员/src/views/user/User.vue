    ...
    <div style="margin-bottom:20px">
      ...
      <el-table :data="tableData" stripe> 
        ...
        <el-table-column prop="username" label="会员卡号" width="80"></el-table-column>
          ...
          <el-table-column prop="updatetime" label="信息编辑时间"></el-table-column>

        <el-table-column label="管理员操作">
            <template v-slot="scope">
                <el-button type="primary" @click="$router.push('/editUser?id='+scope.row.id)">编辑</el-button>
                    <el-popconfirm title="您确定删除吗" @confirm="del(scope.row.id)">
                    <el-button type="danger" slot="reference">删除</el-button>
            </el-popconfirm>
                     <el-popconfirm title="您确定通过申请吗" @confirm="save(scope.row.name)">
                    <el-button type="danger" slot="reference" v-if="scope.row.state !== '已注销' && scope.row.state !== '正常'">确认注销账号</el-button>
            </el-popconfirm>
            </template>
...
<!-- 
            <el-popconfirm title="您确定通过申请吗" @confirm="save(scope.row.name)">
                      <el-button  type="primary" slot="reference" v-if="scope.row.state == '申请注销账号中'">确认注销账号</el-button>
              </el-popconfirm> -->
      </el-table>
      <!-- 分页 -->
...
export default {
  ...
  methods:{
    ...
    save(name){
            request.put('http://localhost:9000/user/update3/'+name).then(res=>{
                if(res.code ==='200'){
