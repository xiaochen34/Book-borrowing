<template>
  <div>
    <div style="margin-bottom:20px">
      ...
      <el-table-column prop="updatetime" label="还书时间"></el-table-column>
      <el-table-column prop="borrowstatus" label="归还状态"></el-table-column>
      <el-table-column prop="statement" label="用户评价"></el-table-column>
        <el-table-column label="操作">
            <template v-slot="scope">
                    <el-popconfirm title="您确定通过申请吗" @confirm="del(scope.row.id)">
                    <el-button style="margin-left:5px" type="danger" slot="reference" v-if="scope.row.borrowstatus === '申请归还中'">确认归还</el-button>
            </el-popconfirm>
             </template>
...
export default {
  ...
  methods:{
    ...
    load(){
      request.get('http://localhost:9000/borrow/page',{params:this.params}).then(res=>{
        if(res.code==='200'){
    ...
    handleCurrentChange(pageNum){
      ...
      this.load()
        ...
        del(id){
        request.delete("http://localhost:9000/borrow/delete2/"+id).then(res=>{
            if(res.code ==='200'){
