...
export default {
  ...
  data(){
    return {
      admin: Cookies.get('category') ? JSON.parse(Cookies.get('category')) : {},
      tableData: [],
      total:0,
  ...
  methods:{
    ...
    load(){
      request.get('http://localhost:9000/category/page',{params:this.params}).then(res=>{
        if(res.code==='200'){
    ...
    handleCurrentChange(pageNum){
      ...
      this.load()
        ...
        del(id){
        request.delete("http://localhost:9000/category/delete/"+id).then(res=>{
            if(res.code ==='200'){
    ...
    save() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          ...
          request.post('http://localhost:9000/category/save', this.form).then(res => {
            if (res.code === '200') {
